...
The central model is the multiorbital Hubbard Hamiltonian,

$$
\begin{aligned}
H = \;  \sum_{ij,m,\sigma} t_{ij}\left( d^{\dagger}_{im\sigma} d_{jm\sigma} + \text{h.c.} \right)
- \mu \sum_{i,m,\sigma} n_{im\sigma} 
+ U \sum_{i,m} n_{im\uparrow} n_{im\downarrow}
+ U' \sum_{i,m\neq m'} n_{im\uparrow} n_{im'\downarrow} + (U' - J) \sum_{i,m<m',\sigma} n_{im\sigma} n_{im'\sigma}
\end{aligned}
$$
